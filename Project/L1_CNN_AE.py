...
for i in range(runMax,-1,-1):
	for j in range(nb_epoch,-1,-1):	
		...
		if os.path.exists('CP/cnnL1AE/cnnL1AE-'+runStr+'-'+epochStr+'.h5'):	
			CPName='CP/cnnL1AE/cnnL1AE-'+runStr+'-'+epochStr+'.h5'
			epoch=j+1
			run=i+1
			break
...
if CPName is None:
	...
	autoencoder.compile(loss='mean_absolute_error', optimizer='adam', metrics=['mean_absolute_error'])
...
checkPointer = ModelCheckpoint(filepath='CP/cnnL1AE/cnnL1AE-'+str(run)+'-{epoch:02d}.h5', verbose=1, save_best_only=False, save_weights_only=False)
